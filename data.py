...
class School:
    ...
    def generate_csv(self, filename: str = "课表.csv") -> str:
        """
        生成CSV格式的课表
        返回生成的文件名
        """
        ...
        for course in self.courses:
            # 直接使用数字表示星期
            weekday_number = course.weekday
            
            # 获取开始和结束节数
            start_index = course.indexes[0]
            end_index = course.indexes[-1]
            
            # 格式化周数信息（只使用数字）
            weeks_str = self._format_weeks_numeric(course.weeks)
    ...
    def _format_weeks_numeric(self, weeks: list[int]) -> str:
        """
        将周数列表格式化为只包含数字的字符串
        例如: [1,2,3,5,6,7,9,10] -> "1-3,5-7,9-10"
        """
        if not weeks:
            return ""
    ...
    def _format_weeks(self, weeks: list[int]) -> str:
        """
        将周数列表格式化为易读的字符串
        例如: [1,2,3,5,6,7,9,10] -> "1-3周, 5-7周, 9-10周"
        """
        if not weeks:
            return ""
        ...
        for i in range(1, len(weeks)):
            if weeks[i] == end + 1:
                ...
                if start == end:
                    groups.append(f"第{start}周")
                else:
                    groups.append(f"第{start}-{end}周")
                start = end = weeks[i]
